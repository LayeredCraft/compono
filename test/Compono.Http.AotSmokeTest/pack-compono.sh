#!/usr/bin/env bash
# Packs Compono and Compono.Http into this project's own local NuGet feed (nuget.config) so the
# AOT publish-and-run proof consumes Compono.Http via an ordinary PackageReference - the same way
...
# TFMs in CI. Mirrors test/Compono.TestDoubles.AotSmokeTest/pack-compono.sh, packing Compono.Http
# instead of Compono.TestDoubles.
set -euo pipefail

script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
feed_dir="$script_dir/../../.local-nuget-feed-http-aot-smoke"
...
# RestorePackagesPath), a rerun after changing Compono/Compono.Http source would silently keep
...
dotnet pack "$script_dir/../../src/Compono.Http/Compono.Http.csproj" -c Release -o "$feed_dir" -p:Version=1.0.0 --nologo
