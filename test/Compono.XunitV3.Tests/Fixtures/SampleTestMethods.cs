...
internal static class SampleTestMethods
{
    ...
    // A by-value (not ref/out/in) ref struct parameter - distinct from WithRefParameter above,
    // which is an ordinary int passed by ref. Span<int> can never legally be a generic type
    // argument to CompositionRow.Resolve<T>()/etc. at all (ADR-0041's dispatch-eligibility guard,
    // runtime side).
