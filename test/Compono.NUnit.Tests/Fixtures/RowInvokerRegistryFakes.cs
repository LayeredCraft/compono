...
/// <see cref="SampleTestMethods.WithNullableParameters"/>, used only for <c>BindingPlan.Build</c>
/// reflection tests) - matching <c>Compono.XunitV3.Tests</c>'/<c>Compono.TUnit.Tests</c>'/
/// <c>Compono.MSTest.Tests</c>' identical hand-fake convention. Real <c>[Compose]</c>-attributed
/// fixture methods (<see cref="SampleTestMethods.ComposesTwoStrings"/> etc.) instead go through the
/// real, generator-discovered registrations (this project's <c>Compono.Generators</c> analyzer
/// reference), so a duplicate fake registration for the same type here is harmless -
/// <c>RowInvokerRegistry.Register</c> is idempotent.
/// </summary>
