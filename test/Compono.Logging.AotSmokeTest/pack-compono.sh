#!/usr/bin/env bash
# Packs Compono and Compono.Logging into this project's own local NuGet feed (nuget.config) so the
# AOT publish-and-run proof consumes Compono.Logging via an ordinary PackageReference - the same way
...
# TFMs in CI. Mirrors test/Compono.Http.AotSmokeTest/pack-compono.sh, packing Compono.Logging
# instead of Compono.Http.
set -euo pipefail

script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
feed_dir="$script_dir/../../.local-nuget-feed-logging-aot-smoke"
...
# RestorePackagesPath), a rerun after changing Compono/Compono.Logging source would silently keep
...
dotnet pack "$script_dir/../../src/Compono.Logging/Compono.Logging.csproj" -c Release -o "$feed_dir" -p:Version=1.0.0 --nologo
