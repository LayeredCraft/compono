#!/usr/bin/env bash
# Packs Compono and Compono.MSTest into this project's own local NuGet feed (nuget.config) so the
# AOT publish-and-run proof consumes Compono.MSTest via an ordinary PackageReference - the same way
...
# part of the published app. No concurrency lock (unlike Compono.MSTest.SampleTests'
# pack-to-local-feed.sh) - this project is a one-shot manual smoke test, not run concurrently across
# TFMs in CI. Mirrors test/Compono.TUnit.AotSmokeTest/pack-compono.sh, packing the same two projects.
set -euo pipefail

script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
feed_dir="$script_dir/../../.local-nuget-feed-mstest-aot-smoke"
...
# RestorePackagesPath), a rerun after changing Compono/Compono.MSTest source would silently keep
...
dotnet pack "$script_dir/../../src/Compono.MSTest/Compono.MSTest.csproj" -c Release -o "$feed_dir" -p:Version=1.0.0 --nologo
