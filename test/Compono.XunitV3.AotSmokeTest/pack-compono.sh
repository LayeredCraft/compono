#!/usr/bin/env bash
# Packs Compono and Compono.XunitV3 into this project's own local NuGet feed (nuget.config) so the
# AOT publish-and-run proof consumes Compono.XunitV3 via an ordinary PackageReference - the same way
...
# part of the published app. No concurrency lock (unlike Compono.XunitV3.SampleTests'
# pack-to-local-feed.sh) - this project is a one-shot manual smoke test, not run concurrently across
# TFMs in CI. Mirrors test/Compono.TUnit.AotSmokeTest/pack-compono.sh and
# test/Compono.MSTest.AotSmokeTest/pack-compono.sh, packing the same two projects for this package.
set -euo pipefail

script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
feed_dir="$script_dir/../../.local-nuget-feed-xunitv3-aot-smoke"
...
# RestorePackagesPath), a rerun after changing Compono/Compono.XunitV3 source would silently keep
...
dotnet pack "$script_dir/../../src/Compono.XunitV3/Compono.XunitV3.csproj" -c Release -o "$feed_dir" -p:Version=1.0.0 --nologo
