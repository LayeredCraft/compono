#!/usr/bin/env bash
# Packs Compono and Compono.XunitV3.Aot into the local NuGet feed this project restores against
# (test/Compono.XunitV3.Aot.SampleTests/nuget.config), serialized behind a cross-process lock, and
# clears this restore's own isolated packages path before every pack. Mirrors
# test/Compono.NUnit.SampleTests/pack-to-local-feed.sh exactly - see that script's own comment for
...
dotnet pack "$xunitv3_aot_csproj" -c "$configuration" -o "$feed_dir" -p:Version=1.0.0 --nologo
