#!/usr/bin/env bash
# Packs Compono and Compono.TUnit into this project's own local NuGet feed (nuget.config) so the AOT
# publish-and-run proof consumes Compono.TUnit via an ordinary PackageReference - the same way any
# real consumer does - rather than a ProjectReference, which propagates dotnet publish's global
...
# part of the published app. No concurrency lock (unlike Compono.TUnit.SampleTests'
# pack-to-local-feed.sh) - this project is a one-shot manual smoke test, not run concurrently across
# TFMs in CI. Mirrors test/Compono.AotSmokeTest/pack-compono.sh, packing one extra project.
set -euo pipefail

script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
feed_dir="$script_dir/../../.local-nuget-feed-tunit-aot-smoke"
...
# RestorePackagesPath), a rerun after changing Compono/Compono.TUnit source would silently keep
...
dotnet pack "$script_dir/../../src/Compono.TUnit/Compono.TUnit.csproj" -c Release -o "$feed_dir" -p:Version=1.0.0 --nologo
