...
/// <see cref="CompositionException"/>, never a guessed resolution. Duplicated from
/// <c>Compono.XunitV3.Binding.ConfigProfileBinder</c>, per this package's own binding-logic
/// duplication decision (see <see cref="PositionalArgumentBinder"/>'s remarks).
/// </summary>
/// <remarks>
/// Every method here is reflection - <see cref="Type.GetConstructors()"/> and
/// <see cref="ConstructorInfo.Invoke(object?[])"/>, not the non-generic <see cref="RowInvokerRegistry"/>-
/// backed dispatch <see cref="RowInvokers"/> uses. That shape exists there to close per-parameter
/// dispatch known only at runtime, once per parameter, so the per-row path never reflects again.
/// <c>TConfig</c>/<c>TProfile</c> need no equivalent: they're already compile-time-closed generic
/// arguments on <see cref="ComposeAttribute{TProfile, TConfig}"/> itself, and this binder's methods
/// are only ever called from <see cref="ComposeAttribute{TProfile, TConfig}.ApplyProfile"/> - itself
/// only ever invoked once per attribute instance, from inside the base <see cref="ComposeAttribute"/>'s
/// existing <c>Lazy&lt;Composer&gt;</c>-backed caching. That existing caching is what bounds this
/// reflection to once per attribute instance, never the repeated per-row data-source path - no
/// separate caching layer is needed here. See ADR-0041 Amendment 1/PLAN-0040 Phase 1's Native AOT gate
/// task for this reflection's own AOT-safety verification.
/// </remarks>
internal static class ConfigProfileBinder
{
    ...
    /// <see cref="ComposeAttribute"/>'s own inline-value binding uses, retargeted at a constructor's
    /// parameters instead of a test method's.
    /// </summary>
    ...
    // exception. ExceptionDispatchInfo.Capture(...).Throw() re-throws the inner exception with its
    // original stack trace preserved, rather than a bare `throw exception.InnerException` (which
    // would reset it) or catching only CompositionException specifically (which would still let a
    // non-CompositionException constructor-thrown exception stay wrongly wrapped).
