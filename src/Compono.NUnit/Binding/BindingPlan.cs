...
/// algorithm reads on every <c>BuildFrom</c> call. Computed exactly once per attribute instance, the
/// first time <c>BuildFrom</c> runs (<see cref="ComposeAttribute"/>'s caching), since the
/// <see cref="System.Reflection.MethodInfo"/> <see cref="Build"/> is given (unwrapped from NUnit's own
/// <c>IMethodInfo</c>) is stable for a given attribute instance. See ADR-0059's binding sections.
/// </summary>
internal sealed class BindingPlan
{
    ...
    /// <see langword="null"/> if the signature is supported. <c>BuildFrom</c> throws using this
    ...
    private static string? ValidateSignature(MethodInfo testMethod, ParameterInfo[] parameters)
    {
        ...
        // the same method without this explicit check - matching Compono.XunitV3/Compono.MSTest.
        ...
        if (testMethod.IsGenericMethodDefinition)
            return $"Compono.NUnit does not support generic test methods ('{methodDisplayName}').";

        foreach (var parameter in parameters)
        {
            if (parameter.ParameterType.IsByRef)
                return $"Compono.NUnit does not support ref/out/in parameters (parameter '{parameter.Name}' on '{methodDisplayName}').";

            if (parameter.GetCustomAttribute<ParamArrayAttribute>() is not null)
                return $"Compono.NUnit does not support params parameters (parameter '{parameter.Name}' on '{methodDisplayName}').";
            ...
            if (parameter.ParameterType.IsByRefLike || parameter.ParameterType.IsPointer)
                return $"Compono.NUnit does not support ref struct or pointer-typed parameters (parameter '{parameter.Name}' on '{methodDisplayName}').";
        }
