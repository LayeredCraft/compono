...
CMP0037 | Compono.TestDoubles | Info | A test-double member appears unimplemented from this interface's own declaration but is actually resolved elsewhere in the closure via an explicit interface reimplementation, a shape Compono's effective-declaration resolution does not yet recognize - the member's unconfigured fallback uses the ordinary computed-default value, which may not match the interface's own resolved behavior (ADR-0044, known open gap)
