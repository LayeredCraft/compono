...
{
    public static TransitiveClosureResult Analyze(ITypeSymbol requestedType, Compilation compilation, LocationInfo? location, GeneratorFeatureFlags flags)
    {
        ...
        if (CollectionWellKnownTypes.GetOrCreate(compilation).TryClassify(requestedType, out _))
            return TransitiveClosureWalker.Walk(requestedType, compilation, location, flags);
        ...
        return TransitiveClosureWalker.Walk(composedType, compilation, location, flags);
    }
