# [ADR-NNNN] Title

**Status:** [Proposed | Accepted | Deprecated | Superseded by ADR-XXXX]

**Date:** YYYY-MM-DD

**Decision Makers:** [List of people involved in the decision, or "solo" if this was a one-person call]

## Context

What is the issue that we're seeing that is motivating this decision or change? Describe the forces at play (technical, product, project-specific).

## Decision Drivers

- [Driver 1, e.g., performance requirement]
- [Driver 2, e.g., API discoverability]
- [Driver 3, e.g., trimming/Native AOT compatibility]
- [Driver 4, e.g., time to a given milestone]

## Considered Options

1. [Option 1]
2. [Option 2]
3. [Option 3]

## Decision Outcome

Chosen option: "[Option X]", because [justification. e.g., best balance of forces, most aligned with constraints].

### Positive Consequences

- [Benefit 1]
- [Benefit 2]

### Negative Consequences

- [Drawback 1, with mitigation if applicable]
- [Drawback 2]

## Pros and Cons of the Options

### [Option 1]

[Description of option 1]

- Good, because [argument a]
- Good, because [argument b]
- Bad, because [argument c]

### [Option 2]

[Description of option 2]

- Good, because [argument d]
- Bad, because [argument e]
- Bad, because [argument f]

### [Option 3]

[Description of option 3]

- Good, because [argument g]
- Bad, because [argument h]

## Links

- [Related ADR or document]
- [External reference or documentation]
- [GitHub issue or PR that triggered this decision]
