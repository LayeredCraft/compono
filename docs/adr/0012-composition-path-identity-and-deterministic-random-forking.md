...
## Amendment 3 (2026-07-29): `ManualResolve` segments for factory/rule-authored resolution

Milestone 3 design review ([ADR-0019](0019-registrations-and-service-provider-injection.md))
adds a descriptor-less `ICompositionContext.Resolve<T>()` overload for hand-written
registration and configuration-rule factories — code that isn't generator-emitted, so
it has no `CompositionRequestDescriptor` to pass. This amendment defines the
`PathSegment` kind that overload produces, and verifies it against this ADR's
reproducibility contract directly, per the design review's explicit request rather
than leaving it as an unverified implementation detail of ADR-0019.

**1. A new segment kind, ordinal-based like `ConstructorParameter`/`RequiredMember`:**
...
a call-sequence counter, scoped to what this amendment calls a **manual-resolve
invocation frame**, stated precisely rather than left as "a per-node counter" (an
earlier design-review draft's looser phrasing, corrected here before implementation):
...
- The frame holds a single mutable counter, starting at `0`. Every descriptor-less
  ...
  counter (first call gets `0`, second gets `1`, and so on), exactly as originally
  specified.
...
No new lifetime concept beyond this: a manual-resolve invocation frame composes with
the ordinary call stack exactly the way path segments and active-construction frames
already do (per PLAN-0002's Execution Flow section) — it exists only because a
factory invocation, unlike a generated plan's `Compose` call, has no descriptor of
its own to hang an ordinal on.

**2. Why ordinal, not requested type, as `ManualResolve`'s identity.** This is the
same reasoning Amendment 1 already applied to `ConstructorParameter`/`RequiredMember`:
a factory that calls `context.Resolve<IClock>()` then `context.Resolve<IRandomService>()`
makes two structurally distinct draws. If `ManualResolve` had no identity beyond "this
is a manual resolve," both calls would derive an identical fork key from the same
parent state — the exact silent-collision bug class Amendment 1 and the original
Fnv1a fix ([PLAN-0002](../plans/0002-milestone-2-core-composition-engine.md) Phase 1
notes) already closed for sibling constructor parameters. Call-sequence ordinal avoids
it the same way constructor-parameter ordinal does, without needing the requested type
(which this ADR's Decision Outcome already bans from hashing) as an input.

**3. Verified against this ADR's reproducibility contract, concretely:**
...
- **Recursion detection still works.** The active-construction-frame stack
  ...
  again is caught by the existing frame-stack check, unmodified. This amendment adds
  no new recursion-detection logic.
- **Reproducible across repeated compositions with the same seed.** `Ordinal` depends
  only on call sequence within one deterministic factory/rule invocation — given the
  same configuration (same registrations/rules, same call graph), two independent
  `Create<T>()` calls with the same seed invoke every factory the same number of
  times, in the same order, producing identical `ManualResolve` ordinal sequences and
  therefore identical fork keys and output. This assumes factories/rules are
  side-effect-free with respect to how many times they call `Resolve<T>()` — the same
  assumption generated code's own determinism already rests on (a generated plan's
  shape is fixed at compile time; a hand-written factory is expected to be equally
  stable across invocations, not conditionally call `Resolve<T>()` a different number
  of times based on external state).
...
- [ADR-0019](0019-registrations-and-service-provider-injection.md) — the
  descriptor-less `Resolve<T>()` overload that Amendment 3 (above) defines
  `PathSegment.ManualResolve` for.
