...
**Planned, not yet implemented: `RowInvokerRegistry` will broaden this
beyond the BCL-only-typed edge case above.** This page otherwise describes
`Compono`'s *current, shipped* dispatch mechanism - the paragraph below is
the one exception, describing
[ADR-0041](../../adr/0041-aot-safe-row-binding-dispatch.md)/
[PLAN-0041](../../plans/0041-aot-safe-row-binding-dispatch.md)'s
...
which ALC its key `Type` or delegate-target assembly came from. Every
registered parameter type will root its generated dispatch delegate (and
the generated assembly defining it), not just the narrow "every composing
type is BCL" case `CollectionPlanCache<T>` above is scoped to. Same
disposition as the collision item and the case above once it ships:
...
related items on this page. This paragraph should be rewritten in plain
present tense (matching the rest of this page) once PLAN-0041 actually
ships `RowInvokerRegistry` - see `tasks/implement.md`'s step 6.
