...
[Throws\(Exception\)](Compono.ReturnConfigBuilder_T_.Throws(System.Exception).md 'Compono\.ReturnConfigBuilder\<T\>\.Throws\(System\.Exception\)') already document\. A literal value or a thrown [System\.Exception](https://learn.microsoft.com/en-us/dotnet/api/system.exception 'System\.Exception') both
implicitly convert to [SequenceOutcome&lt;T&gt;](Compono.SequenceOutcome_T_.md 'Compono\.SequenceOutcome\<T\>'), so a pure\-value sequence reads as plain
values \(`.ReturnsSequence(false, false, true)`\) and a mixed sequence mixes values and
exceptions directly \(`.ReturnsSequence(ex1, ex2, value)`\) \- no separate spelling for either
case\.
