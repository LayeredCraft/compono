...
| [ClearObservedCalls\(\)](Compono.ReturnConfig_T_.ClearObservedCalls().md 'Compono\.ReturnConfig\<T\>\.ClearObservedCalls\(\)') | Clears the recorded call count without changing the configured value, exception, or sequence \- the mirror of [ClearConfiguredResponse\(\)](Compono.ReturnConfig_T_.ClearConfiguredResponse().md 'Compono\.ReturnConfig\<T\>\.ClearConfiguredResponse\(\)')\. Backs `ClearCalls()` \(PLAN\-0063/ ADR\-0060\): observation history is reset, configured behavior \(including in\-progress `Compono.ReturnConfig&lt;&gt;.SequenceOrdinal` progress\) is untouched, so a subsequent call resumes the sequence rather than rewinding it\. |
