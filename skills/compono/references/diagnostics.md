...
| CMP0023 | Whole interface | The interface declares its own `Configure`/`Verify` member that would shadow the generated bridge |
| CMP0024 | Whole interface | A member's generated configuration extension collides with an inherited `object` member (`ToString`/`GetHashCode`/`GetType`; `Equals` collides only for a matching single-required-parameter overload) |
...
| CMP0026 | Whole interface | An unsupported parameter shape: `ref`/`out`/`in`/pointer/function-pointer on a *solo* member (no same-named sibling), or a generic method's own type parameter used as `T?` (constrained or not). Also fires — even with a sibling overload present — for an `out` parameter whose own type has no deterministic default (e.g. a non-nullable reference type): there's no constructible fallback body at all in that case, so it stays whole-interface rather than falling to `CMP0030`'s overload-scoped disposition |
...
| CMP0029 | Colliding identities only | Two or more same-named members (a property vs. a method, or two methods with different real parameter lists) whose generated configuration extensions are each genuinely zero-parameter, so they'd collide (`CS0111`) — the colliding identities fall back; any sibling overload with its own real parameter list is unaffected |
