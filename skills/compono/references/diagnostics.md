...
| CMP0026 | Whole interface | An unsupported parameter shape on a *solo* member (no same-named sibling): `ref`/`out`/`in`, pointer/function-pointer, or a generic method's own type parameter used as `T?` (constrained or not). A `ref`/`out`/`in` parameter on a member *with* a sibling overload is `CMP0030` instead |
...
| CMP0030 | One overload | A `ref`/`out`/`in` parameter on a member that *does* have a same-named sibling — this overload dispatches via a deterministic default with no `Configure()` surface; its sibling overloads are unaffected |
